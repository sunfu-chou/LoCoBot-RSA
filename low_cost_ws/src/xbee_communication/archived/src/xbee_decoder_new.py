...
def xbee_callback(msg_pre):
    ...
    if case == "0":
        ...
        for i in range(info_msg.artifacts.count):
            arti = ArtifactPose()
            arti.Class = strings[19+10*i]
            #arti.status = strings[20+10*i]
            arti.appear_count = int(strings[20+10*i])
            arti.probability = float(strings[21+10*i])

            arti.pose.position.x = float(strings[22+10*i])
            arti.pose.position.y = float(strings[23+10*i])
            arti.pose.position.z = float(strings[24+10*i])

            arti.pose.orientation.x = float(strings[25+10*i])
            arti.pose.orientation.y = float(strings[26+10*i])
            arti.pose.orientation.z = float(strings[27+10*i])
            arti.pose.orientation.w = float(strings[28+10*i])
...
#    pub_apriltags = rospy.Publisher("apriltags_from_xbee", AprilTagDetectionArray, queue_size=1)
    ...
    pub_pose_list = rospy.Publisher("/glora/artifact_global", ArtifactPoseArray, queue_size=1)
