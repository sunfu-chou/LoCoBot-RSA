...
import rospy
from std_msgs.msg import String, Byte, Int64
...
from subt_msgs.srv import node_stat
import sys
...
def cb_ndoe_stat(req):
    print("activated: ", req.count)
    global count 
    count = req.count
...
def cb_timer(self):
    global count
    global VEH
    rospy.loginfo("Encoding node stat message.")
    sent_msg = "8" + VEH[5] + str(count)

    pub_string.publish(sent_msg)
    pub_node_count.publish(count)
...
if __name__ == '__main__':
    ...
    pub_string = rospy.Publisher("xbee_send", String, queue_size=2)
    if VEH == "husky2":
        pub_node_count = rospy.Publisher("node_count_h2", Int64, queue_size=1)
        node_srv = rospy.Service('node_srv', node_stat, cb_ndoe_stat)

        timer = rospy.Timer(rospy.Duration(1), cb_timer)

    if VEH == "husky1":
        pub_node_count = rospy.Publisher("node_count_h1", Int64, queue_size=1)
        node_srv = rospy.Service('node_srv', node_stat, cb_ndoe_stat)
